...
pop <- population$new(
  name = "S_tort",
  generation_time = 1, # in weeks
  size = 1000, # initial population size
  selfing_rate = 0.5, # selfing rate
  mating_system = "random", # random mating
  seedbank_size = 1000, # seed bank size
  seedbank_survival_rate = 0.5, # seed bank survival rate
  seedbank_germination_rate = 0.33 # seed bank germination rate
)
set.seed(123)
pop0 <- population("pop0", time=1000, N=1000)
model <-  compile_model(pop0, generation_time = 1, simulation_length = 1000)
model
system.time(
  ts_pop0 <- msprime(model, sequence_length = 1000000, recombination_rate = 1/1000000)
)
```
