...
## Dissecting the IPMpack function

The IPMpack function `makeIPMFmatrix` ultimate calls `.fecRaw`.  I paste the code from this below to try to understand what is going on.
...
# 80 columns, one for each value of x
...
if (length(which(fecObj@vitalRatesPerOffspringType[, "continuous"] == 1)) > 1) {
  ...
  prodFecValues <- 
    ...
    # now multiple that vector of seed production by the offspring splitter value for "continuous" to keep those seed that are NOT going into the seedbank (1-goSB)
