...
if (length(which(fecObj@vitalRatesPerOffspringType[, "continuous"] == 1)) > 1) {
  ...
  prodFecValues <- 
    ...
    # now multiply that vector of seed production by the offspring splitter value for "continuous" so that the rates reflect seed that are NOT going into the seedbank (1-goSB)
...
```{r}
tmp <- u*h # correct for cell size
plot(u, Fmatrix94[3, -1]) # Compare to IPMpack F Matrix
plot(u, F[2,]) # Compare to JNM F Matrix
```
So, this matches what I hand calculated, but not the final IPM pack F matrix.  So...let's keep working with the code in IPM Matrix.  NEXT STEPS: CODE FROM `makeIPMFmatrix()`
