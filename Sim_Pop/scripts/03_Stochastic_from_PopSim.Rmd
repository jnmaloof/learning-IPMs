...
# 3.1 GB file for 100 gnerations and 100 simulations
...
```{r}
# OExtract summary statistics from all simulations - parallelized version
system.time({
  sim_summary <- future_map_dfr(1:nsims, function(sim_num) {
    simulation.list[[sim_num]] %>% 
      select(-seed) %>%
      unnest(pop) %>%
      filter(week == max(week)) %>% # get year-end info
      group_by(gen) %>%
      summarize(
        ...
        .groups = "drop"  # Avoid group_by warning messages
      ) %>%
...
write_csv(sim_summary, here("Sim_Pop", "output", 
                           str_c("simpop_", nsims, "sims_summary.csv")))
for(i in 2:ngenerations) {
  generations$seed[[i]] <- calc.geno(generations$pop[[i-1]])
  generations$pop[[i]] <- calc.pheno(generations$seed[[i]])
}

saveRDS(generations, file = here("Sim_Pop", "output", str_c("simpop_", ngenerations, "_gen_",  ".Rds")))
