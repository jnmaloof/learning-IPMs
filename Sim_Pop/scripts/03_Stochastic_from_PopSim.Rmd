...
[*] FruitPerPlant should be an integer (model as Poisson)
...
[] record number of seed at each generation
[] Standardize data / column names
[] Maternal parent should be based on seed, not probability
...
[] time dependent mating (can only mate with plants flowering at same time)
[] recombination / more intelligent handling of genotypic effects in mating
[] when we have SNPs, keep track of allele frequencies at the QTL in each generation
...
# calc_geno <- function(pop, size = 2000) {
#   pop <- pop %>% ungroup() %>%
...
predict_size <- function(d, m_size) {
  d %>%
    drop_na(size) %>%
    mutate(size_next = {
      posterior_predict(m_size, newdata = cbind(pick(k_geno, size, Indiv_ID), week), ndraws = 1, allow_new_levels = TRUE) %>%
        magrittr::extract(1, )
    }) %>%
...
calc_pheno <- function(pop, weeks = 12, estab.weeks = 3, m_germ = germ_model_geno, m_estab = estab_model_geno, m_size = size_model_geno, m_surv = surv_model_geno, m_flower = flowering_model_geno, m_fruit = fruit_model_geno, simplify = TRUE) {
  ...
  pred <- pop %>%
    ...
    mutate(
      germinated = {
        posterior_predict(m_germ, newdata = pick(Indiv_ID, germination.logit_geno), ndraws = 1, allow_new_levels = TRUE) %>%
          magrittr::extract(1, ) # note: no need to go to rbinom(), already outputting 0s and 1s
      },
      established =
        ifelse(germinated,
          {
            posterior_predict(m_estab, newdata = pick(Indiv_ID, establishment.logit_geno), ndraws = 1, allow_new_levels = TRUE) %>%
              magrittr::extract(1, )
          },
          NA
...
nsims <- 10
ngenerations <- 10
calc_generations(startingPop = simpop, ngenerations = ngenerations)
```
...
sim_all_data <- future_map_dfr(1:nsims, \(sim) calc_generations(startingPop = simpop, ngenerations = ngenerations, summarize = FALSE),
  .id = "sim",
  .options = furrr_options(seed = TRUE, scheduling = 1)
)


saveRDS(sim_all_data, file = here(
  "Sim_Pop", "output",
  str_c(
