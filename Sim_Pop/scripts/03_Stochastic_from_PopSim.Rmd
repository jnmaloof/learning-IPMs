...
newdata2 <- tibble(
  group = rep_len(LETTERS[14:26], 1000),
  fixef = runif(1000, 0, 1)
)
...
pl1 <- df2 %>% ggplot(aes(x = fixef, y = y)) +
  geom_point() +
  geom_smooth(aes(color = group), se = FALSE, lwd = 1, method = "lm")
ggtitle("original")
...
simpop_timeprep <- simpop %>%
  mutate(
    weeks = as.numeric(week - 1), # Weeks since week 1 (equivalent to pre-transplant or initial size)
    size = if_else(germinated == 0, NA, size) # no size if no germ
  ) %>%
  group_by(Indiv_ID) %>%
  mutate(
    death.week = if_else(y1surv == 1, NA, # if survive year 1 no death week
      first(week, order_by = alive)
    ) # first week where surv = 0
  ) %>%
  mutate(
    size_next = lead(size, order_by = week), # next time point's size)
...
```{r}
germ.model.pheno <- brm(germinated ~ 1,
  family = bernoulli(), data = simpop_timeprep_germ,
  refresh = 0
)
summary(germ.model.pheno)

germ.model.geno <- brm(germinated ~ 1 + germination.logit_geno,
  prior = set_prior("normal(0, 2)", class = "b"),
  family = bernoulli(), data = simpop_timeprep_germ,
  refresh = 0
)
summary(germ.model.geno)
plot(germ.model.geno, ask = FALSE)
```
...
```{r}
estab.model.pheno <- brm(established ~ 1,
  family = bernoulli(), data = simpop_timeprep_estab,
  refresh = 0
)
summary(estab.model.pheno)

estab.model.geno <- brm(established ~ 1 + establishment.logit_geno,
  prior = set_prior("normal(0, 2)", class = "b"),
  family = bernoulli(), data = simpop_timeprep_estab,
  refresh = 0
)

plot(estab.model.geno, ask = FALSE)
summary(estab.model.geno)
```
...
### Phenos Only
```{r}
surv.model.pheno <- brm(alive_next ~ size.scale + (1 | Indiv_ID), # + I(size.scale^2),
  prior = set_prior("normal(0, 10)", class = "b"),
  family = bernoulli(), data = simpop_timeprep_surv,
  cores = 4, refresh = 0
)

plot(surv.model.pheno, ask = FALSE)
summary(surv.model.pheno)
```


```{r}
surv.model.geno <- brm(alive_next ~ size.scale + y1surv.logit_geno + (1 | Indiv_ID), # , + I(size.scale^2)
  prior = set_prior("normal(0, 2)", class = "b"),
  family = bernoulli(),
...
plot(surv.model.geno, ask = FALSE)
summary(surv.model.geno)
```
...
```{r}
size.model.pheno <- brm(size_next ~ 0 + size + week + (0 + size | Indiv_ID),
  prior = set_prior("normal(0, 2)", class = "b"),
...
summary(size.model.pheno)

plot(size.model.pheno, ask = FALSE)

pairs(size.model.pheno)
```

### Geno
should also test without week.
```{r}
size.model.geno <- brm(size_next ~ 0 + size + week + k_geno + (0 + size | Indiv_ID),
  prior = set_prior("normal(0, 2)", class = "b"),
  data = simpop_timeprep_size,
  cores = 4,
  refresh = 0
...
summary(size.model.geno)

plot(size.model.geno, ask = FALSE)
```
## flowering
...
flowering.model.geno <- brm(flowered ~ 1 + flowering.logit_geno,
  prior = set_prior("normal(0, 2)", class = "b"),
  family = bernoulli(), data = simpop_timeprep_flowering,
  refresh = 0
)

plot(flowering.model.geno, ask = FALSE)
summary(flowering.model.geno)
```
...
fruit.model.geno <- brm(fruitPerPlant_pheno ~ fruitPerPlant_geno,
  prior = set_prior("normal(0,2", class = "b"),
...
plot(fruit.model.geno, ask = FALSE)
summary(fruit.model.geno)
```
...
calc.geno <- function(pop, seedPerFruit = 15, maxSeed = 5000) {
  if (nrow(pop) == 0) {
    return(pop)
  } # population is extinct, return empty data frame
...
```{r}
calc.pheno <- function(pop, weeks = 12, estab.weeks = 3, m.germ = germ.model.geno, m.estab = estab.model.geno, m.size = size.model.geno, m.surv = surv.model.geno, m.flower = flowering.model.geno, m.fruit = fruit.model.geno, simplify = TRUE) {
  if (nrow(pop) == 0) {
    return(pop)
  ...
  pred <- pop %>%
    ...
    mutate(
      germinated = {
        posterior_predict(m.germ, newdata = pick(Indiv_ID, germination.logit_geno), ndraws = 1, allow_new_levels = TRUE) %>%
      ...
      established =
        ifelse(germinated,
          ...
          NA
        ), # if no germination, no establishment
  ...
  if (max(pred$germinated, na.rm = TRUE) == 0 ||
    all(is.na(pred$established)) ||
    max(pred$established, na.rm = TRUE) == 0) {
    pred <- pred %>% mutate(
      y1surv = 0,
      flowered = NA,
    ...
    return(pred) # don't try to calculate later phenotypes if no plants germinated or established
  }
  ...
  pred <- pred %>%
    ...
    mutate(
      flowered = pred.flower(pick(Indiv_ID, flowering.logit_geno, week, y1surv), m.flower = flowering.model.geno),
      fruitPerPlant_pheno = pred.fruit(pick(Indiv_ID, fruitPerPlant_geno, week, flowered), m.fruit = fruit.model.geno)
    )
...
saveRDS(sim.all.data, file = here(
  "Sim_Pop", "output",
  str_c(
    "simpop_all_", nsims, "sims_",
    ngenerations, "gen_", Sys.Date(), ".Rds"
  )
))
```
