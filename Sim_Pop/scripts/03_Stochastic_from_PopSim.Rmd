...
```{r}
set.seed(999)
REs <- tibble(group = LETTERS[1:10], re=rnorm(10))
...
newdata <- tibble(group = rep_len(LETTERS[11:20], 1000))
...
df2 <- tibble(value = rnorm(1000, mean = 10, sd = 1),
             group = rep_along(value, LETTERS[1:10]),
...
newdata2 <- tibble(group = rep_len(LETTERS[11:20], 1000),
                   fixef = runif(1000, 0, 1))
...
calc.geno <- function(pop, size = 2000) {
  ...
  pop_filtered <- pop %>% 
    ungroup() %>%
    filter(y1surv == 1, week == max(week)) %>%
    select(ends_with("geno"))
  
  n_pop <- nrow(pop_filtered)
  
  if (n_pop < 2) {
    stop("Need at least 2 individuals in the population to avoid self-mating")
  }
  
  # Generate parent1 indices
  parent1_indices <- sample.int(n_pop, size, replace = TRUE)
  
  # Generate parent2 indices, ensuring they're different from parent1
  parent2_indices <- integer(size)
  for (i in 1:size) {
    # Create a vector of all possible indices except parent1
    available_indices <- setdiff(1:n_pop, parent1_indices[i])
    parent2_indices[i] <- sample(available_indices, 1)
  }
  ...
  # Add individual IDs
  offspring_genos$Indiv_ID <- paste0(1:size, "X") # Adding "X" to distinguish these from the orginal IDs, so that new RE levels are used.
...
```{r}
calc.pheno <- function(pop, weeks = 12, estab.weeks = 3, m.germ = germ.model.geno.brms, m.estab = estab.model.geno.brms, m.size = size.model.geno.brms, m.surv=surv.model.geno.brms, simplify=TRUE) {
  pred <- pop %>%
...
ngenerations <- 100 # note: 4 seconds per generation

generations <- tibble(gen = 1:ngenerations, seed=list(NULL), pop=list(NULL))

generations$seed[[1]] <- calc.geno(simpop)

generations$pop[[1]] <- calc.pheno(generations$seed[[1]])

for(i in 2:ngenerations) {
  generations$seed[[i]] <- calc.geno(generations$pop[[i-1]])
  generations$pop[[i]] <- calc.pheno(generations$seed[[i]])
}

saveRDS(generations, file = here("Sim_Pop", "output", str_c("simpop_", ngenerations, "_gen.Rds")))
...
generations %>% select(-seed) %>%
  ...
  group_by(gen) %>%
  summarize(across(c(germinated, established, y1surv), \(x) mean(x, na.rm = TRUE))) %>%
...
```{r}
#ran this on jetstream m3.xl: 32 CPU, 125GB RAM.  
#if running with full output it will take almost all of the RAM
# if running with simplify = TRUE for calc.pheno and 16 cores, used ~ 36GB RAM, so a m3.l instance should be okay.


nsims <- 100 # number of independent simulations to run
ngenerations <- 100 # note: 4 seconds per generation

# one simulation per list element
simulation.list <- map(1:nsims, \(sim) tibble(gen = 1:ngenerations, seed=list(tibble()), pop=list(tibble())))

#walk(simulation.list, \(sim) print(head(sim, 2)))

# Set up parallel processing
future::plan(future::multisession, workers = 8)

# first generation
# 180 seconds with 8 cores and simplify = TRUE
# 190 seconds with 16 cores with simplify = TRUE
# 290 seconds with 16 cores with simplify = FALSE

system.time({
simulation.list <- future_map(simulation.list, function(sim) {
  sim$seed[[1]] <- calc.geno(simpop)
  sim$pop[[1]] <- calc.pheno(sim$seed[[1]])
  return(sim)
}, .options = furrr_options(seed = TRUE, scheduling = 1))
})

# Loop through all simulations
# 3680 seconds with 16 processors SIMPLIFY = FALSE (Is this for real?)
# or 7000 SIMPLIFY = TRUE, surprised but I guess the simplify operations take time.
# 3824 seconds with 30 processors

future::plan(future::multisession, workers = 30)

system.time({
  # Run simulations in parallel with improved efficiency
  simulation.list <- future_map(simulation.list, function(sim) {
    # Process each generation sequentially
    for(i in 2:ngenerations) {
      # Use parent population to generate next generation
      sim$seed[[i]] <- calc.geno(sim$pop[[i-1]])
      sim$pop[[i]] <- calc.pheno(sim$seed[[i]])
    }
    return(sim)
  }, .options = furrr_options(seed = TRUE, scheduling = 1))
})
...
# 3.1 GB file for 100 generations and 100 simulations
# 1.4 GB with simplify = TRUE
saveRDS(simulation.list, file = here("Sim_Pop", "output", 
                                      str_c("simpop_", nsims, "sims_", 
